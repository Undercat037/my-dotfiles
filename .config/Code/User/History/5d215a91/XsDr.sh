#!/bin/bash
# Переключиться на воркспейс 4 и подождать переключения
hyprctl dispatch workspace 4
sleep 0.3  # Увеличено время ожидания переключения воркспейса
...
sleep 0.4  # Увеличено время для появления окон
...
BOTTOM_X=$((MONITOR_WIDTH - 720 - 1))
BOTTOM_Y=$((MONITOR_HEIGHT - 400 - 1))
...
dispatch moveactive exact 1 51; \
