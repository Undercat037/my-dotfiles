...
# Function to get icon path or name
get_icon_path() {
    local icon_name="$1"
    ...
    if [ -f "/usr/share/icons/hicolor/scalable/apps/${icon_name}.svg" ]; then
        ...
        echo "$icon_name"  # Return icon name for theme lookup
    fi
}
...
# Blacklist with truncated names
BLACKLIST=(
...
while IFS= read -r line; do
    ...
    [ -z "$display_name" ] && continue  # Skip if no name
                ...
                case "$action" in
                    "󰐥 Focus Window")
                        if command -v hyprctl &> /dev/null; then
                            ...
                            [ -n "$address" ] && hyprctl dispatch focuswindow "address:$address" || notify-send "Application Manager" "$selected in background" -i dialog-information
                        fi
                        ;;
                    "󰜺 Minimize")
                        if command -v hyprctl &> /dev/null; then
