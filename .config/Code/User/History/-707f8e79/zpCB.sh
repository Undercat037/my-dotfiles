...
# ──────────────────────────────────────────────────────────────────────────────
# Минимальный фильтр на совсем бесполезные обрезанные имена (можно убрать)
# ──────────────────────────────────────────────────────────────────────────────
should_skip_comm() {
    local comm="$1"
    # Только самые очевидные мусорные имена, которые никогда не бывают GUI
    [[ "$comm" =~ ^(sd-pam|dbus-broker-lau|waitpid|bwrap|wl-paste|crashpad) ]] && return 0
    return 1
}

# ──────────────────────────────────────────────────────────────────────────────
# Сбор приложений — теперь без жёсткого blacklist, только минимальный фильтр
# ──────────────────────────────────────────────────────────────────────────────
> "$TEMP_FILE"
> "$TEMP_MAP"
declare -A seen_apps
        ...
        "🛠 Debug Tools")
            ...
            case "$debug_tool" in
                "All Processes (nice format)")
                    ps -eo user,pid,%cpu,%mem,comm --sort=-%mem | \
                        awk 'NR==1 {print "USER PID %CPU %MEM COMM"} NR>1 {printf "%-10s %-6s %5s%% %5s%% %s\n", $1, $2, $3, $4, $5}' | \
                        rofi -dmenu -i -p "Processes"
                    ;;
                "htop") kitty -e htop & ;;
                "btop") kitty -e btop & ;;
            ...
            if [ -n "$info" ]; then
                ...
                case "$action" in
                    ...
                    "🔥 Unfreeze")
                        ...
                        ;;
                    "🔍 Trace (strace)")
                        confirm=$(printf "Yes\nNo" | rofi -dmenu -i -p "Запустить strace на $selected (PID $pid)? (требуется sudo)")
                        if [ "$confirm" == "Yes" ]; then
