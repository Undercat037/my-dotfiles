...
# ──────────────────────────────────────────────────────────────────────────────
# Минимальный фильтр на бесполезные обрезанные имена
# ──────────────────────────────────────────────────────────────────────────────
should_skip_comm() {
    local comm="$1"
                ...
                "All Processes (nice format)")
                    ps -eo comm,pid,user --sort=-%mem | \
                        awk '
                            ...
                            NR>1 {
                                comm = substr($1, 1, 33)
                                if (length($1) > 33) comm = comm "..."
                                printf "[%-33s] %-10s %-15s\n", comm, $2, $3
                            }
                        ' | \
                        rofi -dmenu -i -p "Все процессы (сортировка по памяти)" -width 70
                    ;;
                "htop") kitty -e htop & ;;
                "btop") kitty -e btop & ;;
