...
BLACKLIST=(
    ...
    "crashpad" "crashpad_handler" "crashpad-google"
)

should_exclude() {
    local comm="$1"
    for item in "${BLACKLIST[@]}"; do [[ "$comm" == "$item" ]] && return 0; done
    [[ "$comm" =~ ^(systemd|gvfs|ibus|fcitx5|xdg|gsd|evolution|tracker|kwin_|plasma|crashpad) ]] && return 0
    return 1
}
        ...
        "🛠 Debug Tools")
            ...
            case "$debug_tool" in
                "All Processes (nice format)")
                    # Красивый формат: [comm] [PID] [USER] %CPU %MEM ...
                    ps -eo user,pid,%cpu,%mem,comm --sort=-%mem | \
                        awk 'NR==1 {print "USER PID %CPU %MEM COMM"} NR>1 {printf "%s %s %s%% %s%% %s\n", $1, $2, $3, $4, $5}' | \
                    ...
                    "🔍 Trace (strace)")
                        ...
                        if [ "$confirm" == "Yes" ]; then
                            ...
                            kitty --title "strace $comm (PID $pid)" -e bash -c "
                                ...
                                read
                            " &
                            notify-send "Trace" "strace запущен. Лог: $log_file" -i utilities-terminal
                        fi
                        ;;
                    "🛠 Debug Tools")
