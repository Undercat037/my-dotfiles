...
# Function to find .desktop file (ТОЛЬКО по имени файла, без жадного Exec)
find_desktop_file() {
...
BLACKLIST=(
    ...
    "mako" "dunst" "swaync"
    "bash" "zsh" "fish" "sh"
    # Добавьте при необходимости: "chrome-sandbox" и т.п.
)
...
# Используем ps -eo для надёжного получения comm
while IFS= read -r user pid comm _; do
    [ "$user" != "$USER" ] && continue
...
done < <(ps -eo user:20,pid:10,comm:50 --sort=-%mem | tail -n +2)  # tail -n +2 убирает header
