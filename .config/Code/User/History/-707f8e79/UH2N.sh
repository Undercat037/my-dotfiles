...
# Build menu using printf with proper formatting
if [ ${#apps[@]} -eq 0 ]; then
    # Debug: show what was filtered
    printf "No GUI applications found\n" > "$TEMP_FILE"
    printf "\n" >> "$TEMP_FILE"
    printf "󰠭 Debug Mode (show all processes)\n" >> "$TEMP_FILE"
else
    > "$TEMP_FILE"
    # Sort and output with proper escaping
    for display_name in $(printf '%s\n' "${!apps[@]}" | sort); do
        icon_name="${apps[$display_name]}"
        icon_path=$(get_icon_path "$icon_name")
        
        if [ -n "$icon_path" ]; then
            # Use null separator for rofi's structured data
...
# Add control options with proper formatting
printf "\n" >> "$TEMP_FILE"
printf "󰚰 Refresh List\0icon\x1fview-refresh\n" >> "$TEMP_FILE"
printf "󰗼 Process Manager\0icon\x1futilities-system-monitor\n" >> "$TEMP_FILE"

# Show rofi menu with proper separator
selected=$(cat "$TEMP_FILE" | rofi -dmenu -i -p "Applications (${#apps[@]})" -show-icons -theme ~/.config/rofi/quick-actions.rasi -markup-rows)
